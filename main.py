...
def main():
    logger = logging.getLogger("xivstrategy")
    logger.setLevel(logging.INFO)

    # create the logging file handler
    logfile = os.path.dirname(os.path.realpath(__file__)) + "/logs/" + "com.skeuo.xivstrategy.log"
    ...
    if SHOULD_SEND_EMAIL:
        ...
        if lastDate.date() == today.date():
            ...
            if ((lastEmailSent is None) or\
                ...
                logger.info("Skip sending email, no new positions")
            dataStore.close()

        else:
            logger.info("Skip sending email, last quote date is not today")
